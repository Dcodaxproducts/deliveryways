...
import { CartResponseDealItem, CartService } from './cart.service';
...
describe('CartService', () => {
  ...
  it('reorders grouped deal rows returned in order items', async () => {
    const { service, ordersService } = makeService();
    const user = {
      id: 'customer-user',
      role: UserRoleEnum.CUSTOMER,
            ...
            includedItems: [
              {
                ...
                modifiers: [
                  ...
                  { modifierId: 'modifier-gyros', quantity: 1 },
                ],
                modifierSelections: [
                  {
    ...
    expect(addItemSpy).toHaveBeenNthCalledWith(
      ...
      expect.objectContaining({
        ...
        modifierSelections: [
          {
            modifierGroupId: 'group-pasta',
            modifiers: [{ modifierId: 'modifier-tagliatelle', quantity: 1 }],
          },
          {
            modifierGroupId: 'group-extra',
            modifiers: [{ modifierId: 'modifier-gyros', quantity: 1 }],
  ...
  it('prices complete fixed combo deal rows at the deal fixed price in cart response', async () => {
    ...
    expect(dealItem.includedItems).toEqual([
      expect.objectContaining({
        menuItemId: 'menu-1',
        type: 'ITEM',
        unitPrice: 80,
  ...
  it('renders different flexible deal selections as separate cart deal entries', async () => {
    const { service, couponsService } = makeService();
    couponsService.getActiveFixedPriceDealPricing.mockResolvedValue({
      dealId: 'deal-flex',
      code: 'FLEX',
    ...
    const grouped = await (
      service as unknown as {
        groupDealItemsForCartResponse: (
          ...
          Array<{ quantity: number; cartItemIds: string[]; lineTotal: number }>
        >;
      }
    ).groupDealItemsForCartResponse(
      [
  ...
  it('removes every included row when removing a fixed combo deal', async () => {
    const { service, cartRepository, profilesRepository } = makeService();
    const cart = {
      id: 'cart-1',
      tenantId: 'tenant-1',
  ...
  it('returns cart without quote when saved address lacks postal code for branch pricing', async () => {
    const { service, cartRepository, profilesRepository, ordersService } =
      makeService();
    cartRepository.findByCustomerId.mockResolvedValue({
      id: 'cart-1',
  ...
  it('returns cart without quote when saved postal code is outside delivery zone', async () => {
    const { service, cartRepository, profilesRepository, ordersService } =
      makeService();
    cartRepository.findByCustomerId.mockResolvedValue({
      id: 'cart-1',
    ...
    profilesRepository.findByUserId.mockResolvedValue({ metadata: {} });
    ordersService.quote.mockRejectedValue(
      new BadRequestException('postal code is not in deliveryzone'),
    );
  ...
  it('returns cart without quote when saved postal code is outside service area', async () => {
    const { service, cartRepository, profilesRepository, ordersService } =
      makeService();
    cartRepository.findByCustomerId.mockResolvedValue({
      id: 'cart-1',
    ...
    ordersService.quote.mockRejectedValue(
      new BadRequestException(
        'Delivery address postal code is not in this service area',
      ),
    );
  ...
  it('returns cart without quote when saved modifiers no longer meet item limits', async () => {
    const { service, cartRepository, profilesRepository, ordersService } =
      makeService();
    cartRepository.findByCustomerId.mockResolvedValue({
      id: 'cart-1',
    ...
    ordersService.quote.mockRejectedValue(
      new BadRequestException(
        'Burger requires at least 1 modifier selection(s)',
      ),
    );
  ...
  it('returns cart without quote when subtotal is below branch minimum order amount', async () => {
    const { service, cartRepository, profilesRepository, ordersService } =
      makeService();
    cartRepository.findByCustomerId.mockResolvedValue({
      id: 'cart-1',
    ...
    profilesRepository.findByUserId.mockResolvedValue({ metadata: {} });
    ordersService.quote.mockRejectedValue(
      new BadRequestException('Subtotal is below branch minimum order amount'),
    );
  ...
  it('returns cart without quote when pickup is unavailable at saved order time', async () => {
    const { service, cartRepository, profilesRepository, ordersService } =
      makeService();
    cartRepository.findByCustomerId.mockResolvedValue({
      id: 'cart-1',
      ...
      couponCode: null,
      paymentMethod: null,
      orderTime: new Date('2026-06-17T09:29:59.848Z'),
      customerNote: null,
      createdAt: new Date(),
      updatedAt: new Date(),
      items: [
  ...
  it('returns cart without quote for unsupported checkout-only quote validation errors', async () => {
    const { service, cartRepository, profilesRepository, ordersService } =
      makeService();
    cartRepository.findByCustomerId.mockResolvedValue({
      id: 'cart-1',
    ...
    profilesRepository.findByUserId.mockResolvedValue({ metadata: {} });
    ordersService.quote.mockRejectedValue(
      new BadRequestException('Selected cart option is not supported'),
    );
  ...
  it('rejects free cart item modifier selections above one', async () => {
    const { service, cartRepository } = makeService();
    const existingCart = {
      id: 'cart-1',
      tenantId: 'tenant-1',
