...
export class CustomerAppController {
  ...
  listCoupons(
    ...
    return this.customerAppService.listCoupons(query, user);
  }
  ...
  listPromotions(
    ...
    return this.customerAppService.listPromotions(query, user);
  }
  ...
  @ApiOperation({ summary: 'Purchase a shareable gift card from wallet' })
  purchaseGiftCard(
    @CurrentUser() user: AuthUserContext,
    @Body() dto: PurchaseGiftCardDto,
    @Query() scope: CustomerAppCustomerScopeDto,
  ) {
    return this.customerAppService.purchaseGiftCard(
      user,
      dto,
      scope.customerId,
    );
