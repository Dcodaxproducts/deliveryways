...
describe('GroupOrdersService', () => {
  ...
  it('stores base item when quote validation retries a stale variation', async () => {
    const { service, groupOrdersRepository, ordersService } = makeService();
    const session = {
      id: 'session-1',
      tenantId: 'tenant-1',
    ...
    groupOrdersRepository.findMenuItemForSession.mockResolvedValue({
      id: 'menu-1',
      branchOverrides: [],
      variations: [{ id: 'stale-variation' }],
    });
    groupOrdersRepository.findMenuItemsForResponse.mockResolvedValue([]);
    ordersService.quoteForCouponValidation
      .mockRejectedValueOnce(
        new BadRequestException('Variation not found for item: test pizza'),
      )
      .mockResolvedValue({ data: { totalAmount: 40, items: [] } });

    await service.addItem(customerUser, 'session-1', {
      menuItemId: 'menu-1',
      variationId: 'stale-variation',
      quantity: 1,
      modifiers: [],
      note: '',
    });
