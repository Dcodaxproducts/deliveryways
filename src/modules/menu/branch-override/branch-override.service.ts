...
export class BranchOverrideService {
  ...
  async upsertItemOverride(user: AuthUserContext, dto: UpsertBranchMenuItemOverrideDto) {
    const item = await this.prisma.menuItem.findUnique({ where: { id: dto.menuItemId } });
    if (!item || item.deletedAt) {
      throw new NotFoundException('Menu item not found');
    }

    const branchId = await this.resolveBranchId(user, dto.branchId, item.restaurantId);
  ...
  async upsertCategoryOverride(user: AuthUserContext, dto: UpsertBranchCategoryOverrideDto) {
    ...
    const branchId = await this.resolveBranchId(user, dto.branchId, category.restaurantId);
  ...
  private async resolveBranchId(
    ...
    if (user.role === UserRoleEnum.BRANCH_ADMIN) {
      ...
      const branch = await this.prisma.branch.findUnique({ where: { id: user.bid } });
      if (!branch || branch.deletedAt) {
        throw new NotFoundException('Branch not found');
      }

      if (branch.restaurantId !== restaurantId) {
        throw new BadRequestException('Item/category does not belong to branch restaurant');
      }
    ...
    if (
      ...
      const branch = await this.prisma.branch.findUnique({ where: { id: requestedBranchId } });
      if (!branch || branch.deletedAt) {
        throw new NotFoundException('Branch not found');
      }

      if (branch.restaurantId !== restaurantId) {
        throw new BadRequestException('Branch does not belong to item/category restaurant');
      }

      if (user.role === UserRoleEnum.BUSINESS_ADMIN && user.rid !== branch.restaurantId) {
    ...
    throw new ForbiddenException('Insufficient permissions for branch override');
  }
}
